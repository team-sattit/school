...
class RoleRepository {
	...
	public function employeeRoleList() {
		$exclude = array(
			config('trt.default_role.admin'),
			config('trt.default_role.student'),
			config('trt.default_role.parent'),
		);

		if (!\Auth::user()->hasRole(config('trt.default_role.admin'))) {
			array_push($exclude, config('trt.default_role.manager'));
			array_push($exclude, config('trt.default_role.principal'));
		}

		return $this->role->whereNotIn('name', $exclude)->get()->pluck('detail', 'id')->prepend(trans('select.role'), '');
	}
